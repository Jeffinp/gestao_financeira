...
export default function Ganhos() {
  ...
  const totalGanhoMes = ganhos
    .filter(g => {
      const dataGanho = new Date(g.data);
      const hoje = new Date();
      return dataGanho.getMonth() === hoje.getMonth() && dataGanho.getFullYear() === hoje.getFullYear();
    })
    .reduce((acc, g) => acc + g.valor, 0);

  // Agrupar ganhos por categoria para o gráfico
  const ganhosPorCategoria = ganhos.reduce((acc, ganho) => {
    if (!acc[ganho.categoria]) {
      acc[ganho.categoria] = 0;
    }
    acc[ganho.categoria] += ganho.valor;
    return acc;
  }, {} as Record<string, number>);

  // Obter top 3 categorias
  const topCategorias = Object.entries(ganhosPorCategoria)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 3)
  ...
  return (
    <div className="page-container">
      ...
      >
        <div>
          <h1 className="text-fluid-3xl font-bold mb-2">Registrar Ganhos</h1>
          <p className="text-muted-foreground">Monitore suas receitas e entrada de valores</p>
        </div>

        <div
          id="success-feedback"
          className="fixed top-20 right-4 bg-secondary/90 text-secondary-foreground px-4 py-3 rounded-lg shadow-lg flex items-center gap-2 transition-opacity duration-300 opacity-0 z-50"
        >
          <CheckCircleIcon className="h-5 w-5" />
          <span>Ganho registrado com sucesso!</span>
        </div>
      </motion.div>
      ...
      >
            ...
            <div>
              ...
              {isLoading ? (
                ...
                <p className="text-2xl font-bold text-secondary mt-1">
                  R$ {totalGanhoMes.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="card hover-lift">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-sm font-medium text-muted-foreground">Principais Fontes</h2>
              {isLoading ? (
        ...
        <div className="card hover-lift">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-medium text-muted-foreground">Último Ganho</h2>
              {isLoading ? (
                <div className="h-8 w-28 bg-muted animate-pulse rounded mt-1"></div>
              ) : ganhos.length > 0 ? (
                <div className="mt-1">
                  <p className="text-base font-medium">
                    {ganhos[0].descricao || ganhos[0].categoria}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-secondary font-medium">
                      R$ {ganhos[0].valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {new Date(ganhos[0].data).toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                </div>
              ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-input bg-card p-2.5 pl-10 text-sm focus:ring-2 focus:ring-secondary focus:border-secondary transition-all duration-200"
                    placeholder="0,00"
                    step="0.01"
                    min="0"
                    required
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className="w-full rounded-lg border border-input bg-card p-2.5 text-sm focus:ring-2 focus:ring-secondary focus:border-secondary transition-all duration-200"
                  placeholder="Detalhes adicionais sobre o ganho..."
                  rows={3}
                />
              </div>
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={handleChange}
                  className="w-4 h-4 text-secondary bg-card border-input rounded focus:ring-secondary transition-all duration-200"
                />
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 bg-secondary text-secondary-foreground font-medium py-2.5 px-4 rounded-lg shadow-sm hover:bg-secondary/90 transition-all duration-200"
              >
          ...
          <div className="card">
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <ArrowUpCircleIcon className="h-12 w-12 text-muted-foreground/40 mb-3" />
                <h3 className="text-lg font-medium mb-1">Nenhum ganho registrado</h3>
                <p className="text-muted-foreground text-sm max-w-md">
                  Registre seu primeiro ganho utilizando o formulário ao lado.
                </p>
              </div>
            ) : (
              <div className="divide-y divide-border/30">
                {ganhos.slice(0, 10).map((ganho, index) => (
                  <div key={index} className="py-3 flex items-start justify-between hover:bg-muted/20 rounded-lg px-2 transition-colors">
                    <div className="flex items-start">
                      ...
                      <div>
                        <h3 className="font-medium">{ganho.descricao || ganho.categoria}</h3>
